...
SecurityAudit::SecurityAudit(const std::string& role,
    ...
    oem_k1_path_ = config.oem_k1_path;
    oem_k2_path_ = config.oem_k2_path;
    oem_kdf_label_ = config.oem_kdf_label.empty() ? "HESIA_OEM_KDF" : config.oem_kdf_label;
    oem_kdf_context_ = config.oem_kdf_context.empty() ? role_ : config.oem_kdf_context;
...
void SecurityAudit::rotate_key_if_requested() {
    if (!enabled_) return;
    if (!oem_k1_path_.empty() && !oem_k2_path_.empty()) {
...
std::vector<uint8_t> SecurityAudit::derive_oem_key_if_available() {
    if (oem_k1_path_.empty() || oem_k2_path_.empty()) {
        return {};
    }
    std::vector<uint8_t> k1 = read_file(oem_k1_path_);
    std::vector<uint8_t> k2 = read_file(oem_k2_path_);
    ...
    const std::string label = oem_kdf_label_.empty() ? "HESIA_OEM_KDF" : oem_kdf_label_;
    const std::string ctx_str = oem_kdf_context_.empty() ? role_ : oem_kdf_context_;
    std::vector<uint8_t> context(ctx_str.begin(), ctx_str.end());
    return kdf_sp800_108_hmac_sha256(key_material, label, context, 32);
}

void SecurityAudit::load_or_create_key(bool rotate) {
    if (!enabled_) return;

    std::vector<uint8_t> oem_key = derive_oem_key_if_available();
    if (!oem_key.empty()) {
        key_ = std::move(oem_key);
        if (logger_) {
            logger_->info("Audit key derived from OEM KDF");
        }
        return;
    }
...
void SecurityAudit::maybe_rotate_key() {
    ...
    if (!oem_k1_path_.empty() && !oem_k2_path_.empty()) return;
