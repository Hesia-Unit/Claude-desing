# AGENTS
- Documents pour agents IA : Codex , Gemini-cli, Claude code

Le Model superieur peut utiliser `gemini-cli` comme pool d'agents de travail.

## Regle generale

- Le Model superieur est l'orchestrateur.
- Les instances `gemini` sont des agents executants.
- Le Model superieur peut ouvrir autant de terminaux que necessaire et lancer `gemini --yolo` dans chacun.
...
- Tu pourra compléter le fichier memories.md pour une memoir persistante entre les conversation et pour reprendre plus tard le travaille 
...
- Le Model superieur théorise et créer un plan de travail pour chaque objectif 
- Le Model superieur decoupe le travail.
- Le Model superieur assigne une tache differente a chaque agent `gemini`.
- Les agents travaillent en parallele.
- Le Model superieur recupere les resultats, tranche, integre, verifie, puis livre le resultat final.

## Discipline

- Un agent par sujet ou sous-probleme.
- Eviter de donner la meme tache a deux agents sauf si Le Model superieur veut comparer deux approches.
- Preferer des consignes concretes: fichier, bug, fonctionnalite, test, revue.
- Le Model superieur garde la responsabilite finale sur l'architecture, la coherence, la qualite, et la livraison.
...
- Matèriel mis a disposition : Huggingface connecter a Le Model superieur
